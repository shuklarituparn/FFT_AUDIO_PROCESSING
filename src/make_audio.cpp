...
#include "../include/make_audio.h"

int main() {
    std::cout
            << "Enter the duration of the audio wave you want to create in seconds";
    std::cin >> duration;
    ...
    if (wav.is_open()) {
        ...
        // after this loop we still don't have the data, we need to populate the
        // chunk
        int end_audio = wav.tellp();
        wav.seekp(
                start_audio -
                4);  // going to the just before the start_audio to write the audio data
        // since the start_audio is 4 bytes we deduct that much
        write_as_bytes(wav, end_audio - start_audio, 4);  // writing the data
        wav.seekp(4, std::ios::beg);  // going to the beginning, and then 4 bytes
        // after it to write the chunk size
